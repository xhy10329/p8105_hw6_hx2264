...
cv_models_compare <- 
  bw_cv %>% 
   mutate(my_model = map(train, ~lm(bwt ~ smoken + ppbmi + gaweeks + delwt + blength + bhead, data = .)),
         ...
         model_3 = map(train, ~lm(bwt ~ bhead + blength + babysex + bhead*babysex + bhead*blength + blength*babysex + bhead*babysex*blength, data = .)))%>% 
   mutate(rmse_my_model = map2_dbl(my_model, test, ~rmse(.x, .y)),
         rmse_model_2 = map2_dbl(model_2, test, ~rmse(.x, .y)),
         rmse_model_3 = map2_dbl(model_3, test, ~rmse(.x, .y)))

cv_rmse_compare %>% 
  gather(key = model, value = rmse, rmse_my_model:rmse_model_3) %>% 
